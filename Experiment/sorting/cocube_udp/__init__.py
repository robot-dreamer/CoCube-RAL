...
class CoCube:
    ...
    def stop(self):
        ...
        self._stopped = True
        self.running = False
        for sock_name in ("sock_listen", "sock_send"):
            ...
            if sock is not None:
                try:
                    ...
                    pass
        thread = getattr(self, "data_process_thread", None)
    ...
    def receive_data_thread(self):
        ...
        while self.running:
            try:
                ...
                if not self.running:
                    break  # Normal shutdown, socket was closed intentionally
                print(f"Error!!!!!: {e}")
                break
...
class Soccer:
    ...
    def stop(self):
        ...
        if self.data_process_thread.is_alive():
            self.data_process_thread.join(timeout=1)
        ...
        while self.running:
            try:
                ...
                if len(self.uuid_results) > 100:         # Control dictionary length to avoid memory overflow
                    ...
                    position = list(map(int, data[1:]))
                    self.pos_p = [position[0], position[1]]
                    self.pos_m = [self.pos_p[0] *1.35*0.001, self.pos_p[1] *1.35*0.001]
                    self.angle = position[2]
                    self.yaw = position[2] * math.pi / 180 if position[2] < 180 else (position[2] - 360) * math.pi / 180
            except socket.timeout:
